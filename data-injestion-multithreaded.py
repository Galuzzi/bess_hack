...
Fixed for psycopg3 compatibility with concurrent processing using 8 processes.
"""
...
class DatabaseConfig(BaseModel):
    ...
    host: str = Field(default="n932lqdpkf.iw7envwgqa.tsdb.cloud.timescale.com")
    port: int = Field(default=35475)
    database: str = Field(default="tsdb")
    username: str = Field(default="tsdbadmin")
    password: str = Field(default="vvlzdfgnlbroz5u1")
...
def get_metric_mapping():
    """Get the metric mapping dictionary - moved to function for multiprocessing"""
    ...
    return {**base_mapping, **cell_mapping, **sensor_mapping}
...
def parse_csv_file(file_path: Path, system_id: int, metric_name: str, subsystem: str) -> List[Tuple]:
...
def process_single_file(file_info: Tuple[Path, int, str, str], db_config: DatabaseConfig, use_copy: bool) -> Tuple[bool, str, int]:
    """Process a single CSV file - designed for multiprocessing"""
    csv_file, system_id, metric_name, subsystem = file_info
    ...
    try:
        ...
        # Parse CSV file
        data_rows = parse_csv_file(csv_file, system_id, metric_name, subsystem)
        if not data_rows:
            return False, f"No valid data in {csv_file.name}", 0
        
        # Insert data using chosen method
        if use_copy:
            success = bulk_insert_metrics_copy(connection, data_rows)
        else:
            success = bulk_insert_metrics(connection, data_rows)
        
        if success:
            return True, f"Successfully processed {csv_file.name}", len(data_rows)
        else:
            return False, f"Failed to insert data from {csv_file.name}", 0
            
    except Exception as e:
        return False, f"Error processing {csv_file.name}: {str(e)}", 0
    finally:
        if connection:
            connection.close()

def bulk_insert_metrics(connection, data_rows: List[Tuple]) -> bool:
...
def bulk_insert_metrics_copy(connection, data_rows: List[Tuple]) -> bool:
...
class BESSDataIngester:
    ...
    def process_csv_files_concurrent(self, system: BESSSystem, use_copy: bool = False, max_workers: int = 8) -> bool:
        """Process all CSV files for a given BESS system using concurrent processing"""
        ...
        logger.info(f"Processing {len(csv_files)} CSV files for system {system.system_name} using {max_workers} processes")
        ...
        for csv_file in csv_files:
            ...
            file_infos.append((csv_file, system_id, metric_name, subsystem))
        ...
        try:
            # Use ProcessPoolExecutor with limited workers to prevent resource exhaustion
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                # Create partial function with fixed arguments
                process_func = partial(process_single_file, db_config=self.db_config, use_copy=use_copy)
                
                # Submit all tasks
                future_to_file = {executor.submit(process_func, file_info): file_info[0] for file_info in file_infos}
                
                # Process completed tasks
                for future in as_completed(future_to_file):
                    csv_file = future_to_file[future]
                    try:
                        success, message, row_count = future.result(timeout=300)  # 5 minute timeout per file
                        if success:
                        ...
                        error_msg = f"Exception processing {csv_file.name}: {str(e)}"
        ...
        logger.info(f"Successfully processed {processed_files}/{len(file_infos)} files")
    ...
    def process_csv_files(self, system: BESSSystem, use_copy: bool = False) -> bool:
        """Process all CSV files for a given BESS system (original sequential method)"""
        system_id = self.get_system_id(system.system_name)
        if not system_id:
            logger.error(f"System {system.system_name} not found in database")
            return False
        ...
        for csv_file in csv_files:
            ...
            if data_rows:
                ...
                insert_method = bulk_insert_metrics_copy if use_copy else bulk_insert_metrics
                
                if insert_method(self.connection, data_rows):
...
def main():
    ...
    parser.add_argument('--host', default='n932lqdpkf.iw7envwgqa.tsdb.cloud.timescale.com', help='Database host')
    parser.add_argument('--port', type=int, default=35475, help='Database port')
    ...
    parser.add_argument('--password', default='vvlzdfgnlbroz5u1', help='Database password')
    ...
    parser.add_argument('--max-workers', type=int, default=8, help='Maximum number of concurrent processes (default: 8)')
    ...
    if args.max_workers < 1:
        ...
        sys.exit(1)
    if args.max_workers > 16:  # Safety limit
        logger.warning(f"max-workers {args.max_workers} is high, limiting to 16")
        args.max_workers = 16
    ...
    try:
        ...
        for system in systems:
            ...
            # Process CSV files - choose method based on command line argument
            if args.sequential:
                logger.info("Using sequential processing")
                ingester.process_csv_files(system, use_copy=args.use_copy)
            else:
                logger.info(f"Using concurrent processing with {args.max_workers} workers")
                ingester.process_csv_files_concurrent(system, use_copy=args.use_copy, max_workers=args.max_workers)
